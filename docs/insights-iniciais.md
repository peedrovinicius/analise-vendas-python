...
Esta página registra os resultados calculados a partir dos arquivos públicos da Olist disponibilizados para o projeto e processados pela camada analítica.
...
A comparação entre anos exige cuidado porque a cobertura de 2016 e 2018 é parcial. Os valores anuais não devem ser comparados sem considerar o número de meses efetivamente cobertos em cada ano.

## Categorias

As categorias com maior receita dos itens foram:

1. **Saúde e beleza** — **R$ 1.233.131,72**.
2. **Relógios e presentes** — **R$ 1.166.176,98**.
3. **Cama, mesa e banho** — **R$ 1.023.434,76**.
4. **Esporte e lazer** — **R$ 954.852,55**.
5. **Computadores e acessórios** — **R$ 888.724,61**.
...
Os 10 maiores vendedores por receita dos itens representam **13,27%** da receita dos pedidos entregues analisados.

O maior vendedor individual representa **1,72%** da receita. A concentração não é, portanto, dominada por um único vendedor na métrica utilizada.

## Recorrência de clientes

Considerando `customer_unique_id`, **2.801 de 93.358 clientes** tiveram mais de um pedido entregue, correspondendo a **3,00%** dos clientes desta visão.

O aprofundamento de segmentação em `docs/insights-negocio.md` detalha a distribuição entre clientes de compra única e recorrentes, além das métricas de frequência e valor.
...
- Relações com pagamentos, avaliações e geolocalização são tratadas separadamente para evitar duplicação de métricas.
